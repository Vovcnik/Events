...
const allSlidesCounter = document.querySelectorAll(
  '.slider-our-spaces__slide'
).length;
...
function initOurFacilitiesSlider() {
  ...
  if (sliderBlocks.length > 0) {
    sliderBlocks.forEach((sliderBlock, index) => {
      ...
      const allOurFacilitiesSliders = document
        .querySelectorAll('.controls-our-facilities__item')
        [index].querySelector('._fraction__all');
      const allCurrentOurFacilitiesSlide = document
        .querySelectorAll('.controls-our-facilities__item')
        [index].querySelector('._fraction__current');
      const allOurFacilitiesSlidersProgress = document
        .querySelectorAll('.controls-our-facilities__item')
        [index].querySelector('.controls-our-facilities__progress');
      const allOurFacilitiesSlidersCounter = slider.querySelectorAll(
        '.our-facilities__slide'
      ).length;
      const sliderItem = new Swiper(slider, {
        ...
        on: {
          ...
          init: function () {
            allOurFacilitiesSliders.innerHTML = setZeroFormat(
              allOurFacilitiesSlidersCounter
            );
          },
          slideChange: function (sl) {
            let currentSlide = ++sl.realIndex;
            allCurrentOurFacilitiesSlide.innerHTML =
              setZeroFormat(currentSlide);
          },
        },
      });
...
tabsOurFacilities.addEventListener('click', function (e) {
  ...
  if (targetElement.classList.contains('tabs-our-facilities__item')) {
    const tabsElements = Array.from(
      document.querySelectorAll('.tabs-our-facilities__item')
    );
    tabsElements.forEach((item) => item.classList.remove('_active'));
    targetElement.classList.add('_active');

    const index = tabsElements.findIndex((item) =>
      item.classList.contains('_active')
    );

    const ourFacilitiesControls = document.querySelectorAll(
      '.controls-our-facilities__item'
    );
    ourFacilitiesControls.forEach((item) => item.classList.remove('_active'));

    const ourFacilitiesSliders = document.querySelectorAll(
...
//========================================================================================================================================================
//========================================================================================================================================================
const adviceSlider = document.querySelector('.advice-slider__body ');
if (adviceSlider) {
  new Swiper(adviceSlider, {
    speed: 800,
    initialSlide: 1,
	 slidesPerView: 1,
    autoHeight: true,
...
const advice2Slider = document.querySelector('.advice-slider__body_2 ');
if (advice2Slider) {
  new Swiper(advice2Slider, {
    speed: 800,
    ...
    // Dotts
    navigation: {
      nextEl: '.advice__arrow_right_2',
      prevEl: '.advice__arrow_left_2',
    },
...
const forestSlider = document.querySelector('.forest__body');
if (forestSlider) {
  new Swiper(forestSlider, {
    speed: 800,
    ...
    navigation: {
      nextEl: '.forest__arrow_right',
      prevEl: '.forest__arrow_left',
    },
...
const PracticeSlider = document.querySelector('.news-designing-practice__items');
if (forestSlider) {
  new Swiper(PracticeSlider, {
    speed: 300,
	 slidesPerView: 3,
	 spaceBetween: 10,
    autoHeight: true,
    ...
    navigation: {
      nextEl: '.practice__arrow_right',
      prevEl: '.practice__arrow_left',
    },
...
if (spollers.length > 0) {
  function spollerCLick(e) {
    const spoller = e.target.classList.contains('_spoller')
      ? e.target
      : e.target.closest('._spoller');
    if (spollersGo) {
      spollersGo = false;
			...
			if (input.classList.contains('_date')) {
				const calendarItem = datepicker(input, {
					...
					formatter: (input, date, instance) => {
						const mouth = instance.months[instance.currentMonth];
						const dateString = String(date);
						const dateArray = dateString.split(' ');
						const newDate = `${dateArray[2]} ${mouth} ${dateArray[3]}`;
